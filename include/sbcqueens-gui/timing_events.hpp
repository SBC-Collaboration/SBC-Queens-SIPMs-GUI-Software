...
class TotalTimeEvent {
 public:
    ...
    TotalTimeEvent<FuncName, Y>& operator=(
        TotalTimeEvent<FuncName, Y>&) = default;
    TotalTimeEvent<FuncName, Y>& operator=(
        TotalTimeEvent<FuncName, Y>&&) = default;
    ...
    auto operator()(Args&&... args) {
        ...
        if constexpr ( std::is_void_v<std::result_of_t<FuncName&&(Args&&...)>> ) {
            ...
            // Calculate the time it has elapsed since last the function
            // was called
            _elapsed_time = std::chrono::duration_cast<time_format>(
                _final_time - _init_time);
            ...
            std::optional<std::invoke_result_t<FuncName, Args...>>
                return_of_func = {};
...
template<typename FuncName, typename Y = std::chrono::milliseconds>
inline TotalTimeEvent<FuncName, Y> make_total_timed_event(
    const Y& total_time, FuncName&& f) {
...
// If the execution time of the function is higher than the function passed
// to it it is essentially ignored.
template<typename FuncName, typename Y = std::chrono::milliseconds>
class BlockingTotalTimeEvent {
 public:
    using time_format = Y;

    BlockingTotalTimeEvent(const time_format& total_time, FuncName&& f)
    ...
    BlockingTotalTimeEvent<FuncName, Y>& operator=(
        BlockingTotalTimeEvent<FuncName, Y>&) = default;
    BlockingTotalTimeEvent<FuncName, Y>& operator=(
        BlockingTotalTimeEvent<FuncName, Y>&&) = default;
    ...
    auto operator()(Args&&... args) {
        ...
        if constexpr (std::is_void_v<std::result_of_t<FuncName&&(Args&&...)>> ) {
            ...
            elapsed_time = std::chrono::duration_cast<time_format>(
                _final_time - _init_time);
...
template<typename FuncName, typename Y = std::chrono::milliseconds>
inline BlockingTotalTimeEvent<FuncName, Y> make_blocking_total_timed_event(
    const Y& total_time, FuncName&& f) {
    return BlockingTotalTimeEvent<FuncName, Y>{total_time,
        std::forward<FuncName>(f)};
}
...
class BlockingFixedTimeEvent {
 public:
    ...
    BlockingFixedTimeEvent<FuncName, Y>& operator=(
        BlockingFixedTimeEvent<FuncName, Y>&&) = default;
...
template<typename FuncName, typename Y = std::chrono::milliseconds>
inline BlockingFixedTimeEvent<FuncName, Y> make_blocking_fixed_timed_event(
    const Y& total_time, FuncName&& f) {
    return BlockingFixedTimeEvent<FuncName, Y>{total_time,
        std::forward<FuncName>(f)};
}
